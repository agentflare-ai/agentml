...
LLM outputs are constrained to structured JSON events validated against schemas using the `schema` attribute. This ensures reliability and type safety.

```xml
<transition event="intent.flight"
            schema='{"type": "object", "properties": {"action": {"type": "string", "enum": ["search", "book", "cancel"]}}}'
...
* 📝 **Schema-Guided Events**: `schema` attributes validate LLM-generated events
...
1. **Document Structure**: AgentML files (`.aml`) use an `<agentml>` root element, which extends SCXML's `<scxml>` element.
