...
description: "Understanding AgentML document structure with the <agentml> root element"
---

# Document Structure

AgentML documents use XML syntax based on W3C SCXML, with an `<agentml>` root element that extends the standard `<scxml>` specification. Understanding this structure is essential for building effective agents.

## The `<agentml>` Root Element

The `<agentml>` element is the root of every AgentML document. It extends SCXML's `<scxml>` element with AgentML-specific features:
...
4. **Define schemas for all events**: Use `schema` for type safety
