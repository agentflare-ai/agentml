...
In AgentML, namespaces extend the base SCXML vocabulary with domain-specific actions. They are declared using the `xmlns:prefix="uri"` directive on the `<agentml>` root element:
