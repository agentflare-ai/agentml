...
The `schema` attribute minimizes token usage by providing precise output specifications:
...
3. **Schema Everything**: Use `schema` for all LLM outputs
...
<agentml xmlns="github.com/agentflare-ai/agentml"
  ...
  <state id="collect_info">
    <transition event="user.message"
                schema='{"type": "object", "properties": {"from": {"type": "string"}, "to": {"type": "string"}, "date": {"type": "string"}}}'
                target="search">
