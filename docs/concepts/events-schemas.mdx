...
description: "Structure your data with schema validation for type-safe AgentML agents"
---

# Events & Schemas

AgentML uses JSON Schema to validate events, ensuring type safety and guiding LLMs to generate correctly structured data. The `schema` attribute is a key feature that makes agents reliable and predictable.
...
The `schema` attribute validates events against JSON Schema, providing:
...
<agentml xmlns="github.com/agentflare-ai/agentml"
  ...
  <state id="classify_intent">
    ...
    <transition event="intent.greeting"
                schema="events:#/components/schemas/GreetingIntent"
...
<agentml xmlns="github.com/agentflare-ai/agentml"
  ...
  <transition event="user.message"
              schema="api:#/components/schemas/UserMessage"
              target="processing" />
</agentml>
```
