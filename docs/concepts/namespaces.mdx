...
Namespaces extend the SCXML vocabulary with custom actions and elements. In AgentML, namespaces are declared using the `xmlns:prefix="uri"` directive on the `<agentml>` root element.
...
<agentml xmlns="github.com/agentflare-ai/agentml"
  ...
  <!-- schema is from the agent namespace -->
...
<agentml xmlns="github.com/agentflare-ai/agentml"
  ...
  <transition event="user.message"
              schema="events:#/components/schemas/UserMessage"
              target="processing" />
</agentml>
```
