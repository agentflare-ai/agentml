...
Combine with `schema` for structured outputs:
  ...
  <state id="classify">
    <onentry>
      <openai:generate
        model="gpt-4o"
        location="_event"
        promptexpr="'Classify this text: ' + userInput" />
    </onentry>

    <transition event="action.response"
                schema='{"type": "object", "properties": {"class": {"type": "string"}}}'
                target="handle_classification" />
  </state>
</agentml>
```
...
4. **Use schemas**: Guide output structure with `schema`
