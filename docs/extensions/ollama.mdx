...
<agentml xmlns="github.com/agentflare-ai/agentml"
  ...
  <state id="classify_local">
    <onentry>
      <!-- Use local model for classification -->
      <ollama:generate
        model="llama2"
        location="_event"
        promptexpr="'Classify as positive or negative: ' + text" />
    </onentry>

    <transition event="intent.positive" target="handle_positive" />
    <transition event="intent.negative" target="handle_negative" />
  </state>

  <!-- Fallback to cloud for complex tasks -->
  <state id="generate_complex">
    <onentry>
      <openai:generate
        model="gpt-4o"
        location="_event"
        promptexpr="'Generate detailed response: ' + userInput" />
    </onentry>
